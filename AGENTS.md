...
5. Never commit planning, design, or process artifacts generated by agent or
   skill workflows. Keep those temporary materials local and ignored.
...
- `main` is the only long-lived branch. All changes use a short-lived typed
  branch and a Pull Request; never push directly to `main`.
- Keep each Pull Request focused on one independently reviewable outcome and
  rebase it onto the latest `origin/main` before merge.
- Merge only after required CI passes, every discussion is resolved, and one
  non-author reviewer approves. Priority does not bypass these requirements.
- Use Squash merge only. The Conventional Commit Pull Request title becomes the
  commit on `main`, and the source branch is deleted after merge.
- Revert through a new Pull Request. Never rewrite `main` or move, replace, or
  delete a published tag or Release.
- Follow [`CONTRIBUTING.md`](./CONTRIBUTING.md) for the complete human workflow.
